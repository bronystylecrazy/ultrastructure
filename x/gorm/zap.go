...
func WithParamFilter(on bool) LoggerOption {
	return func(l *Logger) { l.ParameterizedQueries = on }
}

func NewGormLogger(zapLogger *zap.Logger, opts ...LoggerOption) Logger {
	l := Logger{
		ZapLogger:                 zapLogger,
...
var (
	...
	ultrastructurePackage = filepath.Join("ultrastructure", "x", "gorm")
)
...
func (l Logger) logger(ctx context.Context) *zap.Logger {
	...
	for i := 2; i < 15; i++ {
		...
		case strings.Contains(file, ultrastructurePackage):
		default:
