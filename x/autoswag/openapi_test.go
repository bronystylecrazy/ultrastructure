...
func TestBuildOpenAPISpecWithOptions_IncludesExtraModelsInComponents(t *testing.T) {
	...
	if _, ok := spec.Components.Schemas["autoswag.extraSwaggerModel"]; !ok {
		t.Fatalf("expected autoswag.extraSwaggerModel schema to be included in components")
	}
}
...
func TestBuildOpenAPISpec_DefaultModelSchemaNameIncludesPackage(t *testing.T) {
	GetGlobalRegistry().Clear()
	GetGlobalRegistry().RegisterRoute("GET", "/errors", &RouteMetadata{
		Responses: map[int]ResponseMetadata{
			400: {
				Type:        reflect.TypeOf(web.Error{}),
	...
	getOp := spec.Paths["/errors"]["get"].(map[string]interface{})
	responses := getOp["responses"].(map[string]interface{})
	resp400 := responses["400"].(map[string]interface{})
	content := resp400["content"].(map[string]interface{})
	jsonContent := content[web.ContentTypeApplicationJSON].(map[string]interface{})
	schema := jsonContent["schema"].(map[string]interface{})
	if schema["$ref"] != "#/components/schemas/web.Error" {
		t.Fatalf("expected package-qualified schema ref, got %v", schema["$ref"])
	}
}

func TestBuildOpenAPISpec_GeneratesMultipleResponseMediaTypes(t *testing.T) {
	GetGlobalRegistry().Clear()
	...
	GetGlobalRegistry().RegisterRoute("GET", "/multi-content", &RouteMetadata{
		Responses: map[int]ResponseMetadata{
			200: {
				Content: map[string]reflect.Type{
					"text/plain":       reflect.TypeOf(""),
					"application/json": reflect.TypeOf(jsonResp{}),
				},
	...
	spec := BuildOpenAPISpec([]RouteInfo{
		{Method: "GET", Path: "/multi-content"},
	}, Config{Name: "Test API"})

	getOp, ok := spec.Paths["/multi-content"]["get"].(map[string]interface{})
	if !ok {
		t.Fatalf("expected GET /multi-content operation")
	}
	...
	plain := content["text/plain"].(map[string]interface{})
	plainSchema := plain["schema"].(map[string]interface{})
	if plainSchema["type"] != "string" {
		t.Fatalf("expected text/plain schema type string, got %v", plainSchema["type"])
	}

	jsonMedia := content["application/json"].(map[string]interface{})
	jsonSchema := jsonMedia["schema"].(map[string]interface{})
	if _, ok := jsonSchema["$ref"]; !ok {
		t.Fatalf("expected application/json schema to be a $ref, got %v", jsonSchema)
	}
}
...
func TestBuildOpenAPISpec_GeneratesMultiSchemaRequestBodyFromContentMap(t *testing.T) {
	GetGlobalRegistry().Clear()
	...
	GetGlobalRegistry().RegisterRoute("POST", "/users/multi-schema", &RouteMetadata{
		RequestBody: &RequestBodyMetadata{
			Required: true,
			Content: map[string]reflect.Type{
				"application/json":                  reflect.TypeOf(jsonReq{}),
				"application/x-www-form-urlencoded": reflect.TypeOf(formReq{}),
			},
		},
	})

	spec := BuildOpenAPISpec([]RouteInfo{
		{Method: "POST", Path: "/users/multi-schema"},
	}, Config{Name: "Test API"})

	postOp := spec.Paths["/users/multi-schema"]["post"].(map[string]interface{})
	...
	formContent := content["application/x-www-form-urlencoded"].(map[string]interface{})
	formSchema := formContent["schema"].(map[string]interface{})
	if _, ok := formSchema["$ref"]; !ok {
		t.Fatalf("expected form schema to be a $ref, got %v", formSchema)
	}
}
...
func TestBuildOpenAPISpecWithOptions_AddsInfoMetadataAndTagDescriptions(t *testing.T) {
	...
	spec := BuildOpenAPISpecWithOptions([]RouteInfo{
		...
		License: &OpenAPILicense{
			...
			URL:  "https://example.com/licenses/private-use",
		},
	...
	if spec.Info.License == nil || spec.Info.License.Name != "Private Use Only" {
...
func TestBuildOpenAPISpec_UsesHandlerPackageNameAsTagFallback(t *testing.T) {
	GetGlobalRegistry().Clear()
	...
	spec := BuildOpenAPISpec([]RouteInfo{
		{
			Method:             "GET",
			Path:               "/",
			HandlerPackagePath: "github.com/acme/service/internal/users",
		},
	}, Config{Name: "Test API"})

	getRoot := spec.Paths["/"]["get"].(map[string]interface{})
	tags, ok := getRoot["tags"].([]string)
	if !ok {
		t.Fatalf("expected []string tags, got %T", getRoot["tags"])
	}
	if len(tags) != 1 || tags[0] != "users" {
		t.Fatalf("expected handler package fallback tag [users], got %v", tags)
	}

	if len(spec.Tags) != 1 || spec.Tags[0].Name != "users" {
		t.Fatalf("expected top-level tags to include users, got %+v", spec.Tags)
	}
}

func TestBuildOpenAPISpec_MetadataTagsOverrideHandlerPackageFallback(t *testing.T) {
	GetGlobalRegistry().Clear()
	GetGlobalRegistry().RegisterRoute("GET", "/", &RouteMetadata{
		Tags: []string{"System"},
	})
	...
	if len(tags) != 1 || tags[0] != "System" {
		t.Fatalf("expected metadata tag [System], got %v", tags)
	}
}

func TestBuildOpenAPISpec_TransformsHandlerPackageFallbackTag(t *testing.T) {
	GetGlobalRegistry().Clear()
	...
	spec := BuildOpenAPISpecWithOptions([]RouteInfo{
		{
			Method:             "GET",
			Path:               "/",
			HandlerPackagePath: "github.com/acme/service/internal/users_api",
		},
	}, Config{Name: "Test API"}, OpenAPIBuildOptions{
		PackageTagTransform: func(tag string) string {
	...
	if len(tags) != 1 || tags[0] != "USERSAPI" {
		t.Fatalf("expected transformed tag [USERSAPI], got %v", tags)
	}
}
...
func TestBuildOpenAPISpec_GeneratesDeterministicOperationIDsWithPathContext(t *testing.T) {
	ClearOperationIDHook()
	defer ClearOperationIDHook()
	GetGlobalRegistry().Clear()

	spec := BuildOpenAPISpec([]RouteInfo{
		{Method: "GET", Path: "/api/v1/px/users/:id_or_uuid/pooler"},
	}, Config{Name: "Test API"})

	getPooler := spec.Paths["/api/v1/px/users/{id_or_uuid}/pooler"]["get"].(map[string]interface{})
	if getPooler["operationId"] != "getPxUserPoolerByIdOrUuid" {
		t.Fatalf("unexpected operationId for GET /api/v1/px/users/{id_or_uuid}/pooler: %v", getPooler["operationId"])
	}
}
...
func TestBuildOpenAPISpec_OperationIDUniquenessWithSuffix(t *testing.T) {
	...
	if getAdmins["operationId"] != "customIDV2" {
...
func TestWithSwaggerCustomize_ComposesMultipleHooks(t *testing.T) {
	...
	cfg := options{}
	WithCustomize(func(ctx *Context) {
		ctx.Metadata.Tags = []string{"Users"}
	})(&cfg)
	WithCustomize(func(ctx *Context) {
...
func TestComposeSwaggerCustomizeHook_AppliesDICustomizersAfterBaseHook(t *testing.T) {
	...
	customizers := []Customizer{
		CustomizeFunc(func(ctx *SwaggerContext) {
			ctx.Metadata.OperationID = ctx.Metadata.Tags[0] + "__" + ctx.Metadata.OperationID
		}),
		CustomizeFunc(func(ctx *SwaggerContext) {
			ctx.Models.AddNamed("CustomTest", struct {
...
func TestComposeSwaggerCustomizeHooks_RunsPreAndPostWhenImplemented(t *testing.T) {
	...
	preCustomizers := []PreRun{customizers[0].(PreRun)}
	postCustomizers := []PostRun{customizers[0].(PostRun)}
...
func TestSwaggerContext_SetResponseAs_MergesContentTypes(t *testing.T) {
	ctx := &SwaggerContext{
		Method: "GET",
		Path:   "/merge",
		Metadata: &RouteMetadata{
	...
	ctx.SetResponseAs(200, web.Response{}, "application/json", "json response")
	ctx.SetResponseAs(200, "", "text/plain", "plain response")

	resp := ctx.Metadata.Responses[200]
	if len(resp.Content) != 2 {
		t.Fatalf("expected 2 content entries, got %+v", resp.Content)
	}
	if _, ok := resp.Content["application/json"]; !ok {
		t.Fatalf("expected application/json content to be retained")
	}
	if _, ok := resp.Content["text/plain"]; !ok {
		t.Fatalf("expected text/plain content to be added")
	}
}

func TestBuildOpenAPISpecWithOptions_ConflictDiagnostics(t *testing.T) {
	GetGlobalRegistry().Clear()
	GetGlobalRegistry().RegisterRoute("GET", "/conflict", &RouteMetadata{
		RequestBody: &RequestBodyMetadata{
			Type:     reflect.TypeOf(searchUsersBody{}),
			Required: true,
			Content: map[string]reflect.Type{
				"application/json": reflect.TypeOf(searchUsersBody{}),
			},
		},
		Responses: map[int]ResponseMetadata{
			200: {
				Type:        reflect.TypeOf(web.Response{}),
				ContentType: "application/json",
				Content: map[string]reflect.Type{
					"application/json": reflect.TypeOf(web.Response{}),
				},
	...
	spec := BuildOpenAPISpecWithOptions([]RouteInfo{
		...
		Hook: func(ctx *SwaggerContext) {
			...
			ctx.SetResponseAs(200, struct {
				ID string `json:"id"`
			}{}, "application/json", "auto-detected")
		},
	})

	op := spec.Paths["/conflict"]["get"].(map[string]interface{})
	rawWarnings, ok := op["x-autoswag-warnings"]
	if !ok {
		t.Fatalf("expected x-autoswag-warnings to be present")
	}
	warnings, ok := rawWarnings.([]string)
	if !ok || len(warnings) == 0 {
		t.Fatalf("expected non-empty warnings, got %T %#v", rawWarnings, rawWarnings)
	}
	rawDiagnostics, ok := op["x-autoswag-diagnostics"]
	if !ok {
		t.Fatalf("expected x-autoswag-diagnostics to be present")
	}
	diagnostics, ok := rawDiagnostics.([]map[string]interface{})
	if !ok || len(diagnostics) == 0 {
		t.Fatalf("expected non-empty diagnostics, got %T %#v", rawDiagnostics, rawDiagnostics)
	}
...
func TestBuildOpenAPISpecWithOptions_FailOnConflictDiagnostics(t *testing.T) {
	GetGlobalRegistry().Clear()
	GetGlobalRegistry().RegisterRoute("GET", "/conflict-fail", &RouteMetadata{
		Responses: map[int]ResponseMetadata{
			200: {
				Type:        reflect.TypeOf(web.Response{}),
				ContentType: "application/json",
	...
	defer func() {
		if recover() == nil {
			t.Fatalf("expected panic when fail-on-diagnostics is enabled")
		}
	}()

	_ = BuildOpenAPISpecWithOptions([]RouteInfo{
		{Method: "GET", Path: "/conflict-fail"},
	}, Config{Name: "Test API"}, OpenAPIBuildOptions{
		IncludeDiagnostics:  true,
