...
type schemaUserStatus string

const (
	schemaStatusActive  schemaUserStatus = "active"
	schemaStatusPending schemaUserStatus = "pending"
)

type schemaPriorityLevel int

const (
	schemaPriorityLow  schemaPriorityLevel = 1
	schemaPriorityHigh schemaPriorityLevel = 5
)

type enumBackedSchema struct {
	Status   schemaUserStatus    `json:"status"`
	Priority schemaPriorityLevel `json:"priority"`
}
...
func TestSchemaExtractor_AppliesRegisteredEnumsForCustomTypes(t *testing.T) {
	...
	RegisterEnum(schemaStatusActive, schemaStatusPending)
	RegisterEnum(schemaPriorityLow, schemaPriorityHigh)
	...
	if len(statusEnum) != 2 || statusEnum[0] != schemaStatusActive || statusEnum[1] != schemaStatusPending {
	...
	if len(priorityEnum) != 2 || priorityEnum[0] != schemaPriorityLow || priorityEnum[1] != schemaPriorityHigh {
...
func TestSchemaExtractor_DeduplicatesSameTypeNameAcrossPackages(t *testing.T) {
	...
	schemas := extractor.GetSchemas()
	if _, ok := schemas["pkgalpha.Duplicate"]; !ok {
		t.Fatalf("expected first schema key pkgalpha.Duplicate")
	}
	if _, ok := schemas["pkgbeta.Duplicate"]; !ok {
		t.Fatalf("expected schema key pkgbeta.Duplicate, got keys=%v", reflect.ValueOf(schemas).MapKeys())
	}
}
