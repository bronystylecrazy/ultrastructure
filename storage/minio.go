...
func (s *MinioStorage) Start(ctx context.Context) error {
	client, err := CreateMinioClient(ctx, s.config, s.L())
	if err != nil {
		return err
	}
	s.client = client
...
func (s *MinioStorage) Upload(ctx context.Context, reader io.Reader, opts *UploadOptions) (*Metadata, error) {
	...
	if opts.ExtractImageDimensions {
		...
		if _, err := io.Copy(buf, reader); err != nil {
			s.L().Warn("Failed to buffer for dimension extraction", zap.Error(err))
		} else {
	...
	if err != nil {
		s.L().Error("Failed to upload", zap.String("key", key), zap.Error(err))
	...
	s.L().Info("Upload successful", zap.String("key", key), zap.Int64("size", info.Size))
...
func (s *MinioStorage) Download(ctx context.Context, key string, opts *DownloadOptions) (io.ReadCloser, error) {
	...
	if opts.Range != "" {
		if err := getOpts.SetRange(0, 0); err != nil {
			...
			s.L().Warn("Failed to set range", zap.Error(err))
		}
	}

	obj, err := s.client.GetObject(ctx, s.config.BucketName, key, getOpts)
	if err != nil {
		s.L().Error("Failed to download", zap.String("key", key), zap.Error(err))
...
func (s *MinioStorage) Delete(ctx context.Context, key string, opts *DeleteOptions) error {
	...
	if opts.Recursive {
		...
		for err := range errorCh {
			if err.Err != nil {
				s.L().Error("Failed to delete object", zap.String("key", err.ObjectName), zap.Error(err.Err))
				return fmt.Errorf("failed to delete %s: %w", err.ObjectName, err.Err)
			}
		}

		s.L().Info("Deleted recursively", zap.String("prefix", key))
		return nil
	}

	err := s.client.RemoveObject(ctx, s.config.BucketName, key, minio.RemoveObjectOptions{})
	if err != nil {
		s.L().Error("Failed to delete", zap.String("key", key), zap.Error(err))
		return fmt.Errorf("failed to delete %s: %w", key, err)
	}

	s.L().Info("Deleted", zap.String("key", key))
	return nil
}
...
func (s *MinioStorage) GetMetadata(ctx context.Context, key string) (*Metadata, error) {
	...
	if err != nil {
		s.L().Error("Failed to get metadata", zap.String("key", key), zap.Error(err))
...
func (s *MinioStorage) List(ctx context.Context, opts *ListOptions) (*ListResult, error) {
	...
	for object := range objectCh {
		if object.Err != nil {
			s.L().Error("List error", zap.Error(object.Err))
...
func (s *MinioStorage) GetSignedURL(ctx context.Context, key string, expiry time.Duration) (string, error) {
	...
	if err != nil {
		s.L().Error("Failed to generate signed URL", zap.String("key", key), zap.Error(err))
