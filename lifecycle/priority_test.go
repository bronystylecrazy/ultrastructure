...
func TestSortStartersByPriorityStable(t *testing.T) {
	...
	c := &testStarter{id: "c"}
	d := &testStarter{id: "d"}
	di.RegisterMetadata(b, startPriority{Index: int(Earlier)})
	di.RegisterMetadata(c, startPriority{Index: int(Earlier)})
	di.RegisterMetadata(d, startPriority{Index: int(Later)})
...
func TestSortStoppersByPriorityStable(t *testing.T) {
	...
	c := &testStopper{id: "c"}
	d := &testStopper{id: "d"}
	di.RegisterMetadata(b, stopPriority{Index: int(Earlier)})
	di.RegisterMetadata(c, stopPriority{Index: int(Earlier)})
	di.RegisterMetadata(d, stopPriority{Index: int(Later)})
...
func TestStartPriorityLastMetadataWins(t *testing.T) {
	s := &testStarter{id: "s"}
	di.RegisterMetadata(s, startPriority{Index: int(Earlier)}, startPriority{Index: int(Latest)})
	if got := startPriorityIndex(s); got != int(Latest) {
		t.Fatalf("unexpected start priority index: got %d want %d", got, int(Latest))
	}
}

func TestStopPriorityLastMetadataWins(t *testing.T) {
	s := &testStopper{id: "s"}
	di.RegisterMetadata(s, stopPriority{Index: int(Earlier)}, stopPriority{Index: int(Latest)})
	if got := stopPriorityIndex(s); got != int(Latest) {
		t.Fatalf("unexpected stop priority index: got %d want %d", got, int(Latest))
	}
}
