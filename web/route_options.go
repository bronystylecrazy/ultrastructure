...
func Query[T any]() RouteOption {
	var zero T
	return func(b *RouteBuilder) *RouteBuilder {
		// Keep parity with RouteBuilder.Query behavior and infer via reflect.TypeOf.
		return b.Query(zero)
	}
}
...
func zeroValueForType[T any]() any {
	...
	// For pointer types we need a non-nil value to preserve pointer type through reflect.TypeOf.
